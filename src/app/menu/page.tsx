...
export default function MenuPage() {
  ...
  useEffect(() => {
    ...
    if (menus.length > 0) {
      ...
      if (idFromQuery && menus.some((m) => m.id === idFromQuery)) {
        setSelectedMenuId(idFromQuery);
  ...
  useEffect(() => {
    ...
    const menu = loadMenuVersion(selectedMenuId);
    if (menu) {
  ...
  const handleDeleteMenu = useCallback(() => {
    if (confirm("Tem certeza que deseja deletar este cardápio?")) {
      deleteMenuVersion(selectedMenuId);
      const menus = getSavedMenus();
      setSavedMenus(menus);
      if (menus.length > 0) {
        setSelectedMenuId(menus[0].id);
  ...
  const handleFileImport = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (selectedMenuId) {
          updateMenuVersion(selectedMenuId, menu.name, menu.items, menu.pricingRules);
          setMenuItems(menu.items);
          setPricingRules(menu.pricingRules);
          setMenuName(menu.name);
          setError(null);
          const originalError = error;
          setError("Cardápio importado e salvo com sucesso!");
          setTimeout(() => setError(originalError), 3000);
        } else {
          setError("Selecione um cardápio primeiro.");
        }
      } catch (e) {
        const message =
  ...
  const handleImportFromClipboard = useCallback(async () => {
    ...
    try {
      const menu = await importMenuFromClipboard();
        ...
        <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-8 px-4 py-10 sm:px-8 lg:px-12">
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <p className="text-zinc-700 dark:text-zinc-300">
              Nenhum cardápio encontrado. Crie um cardápio primeiro.
            </p>
            <button
              type="button"
              onClick={() => router.push("/menu/create")}
              className="mt-4 rounded-lg bg-black px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900"
            >
              Criar cardápio
            </button>
          </div>
    ...
    <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900 dark:bg-black dark:text-zinc-50">
      <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-8 px-4 py-10 sm:px-8 lg:px-12">
        <header className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-zinc-600 dark:text-zinc-400">
                Selecione o cardápio e os itens desejados para gerar seu pedido formatado.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => router.push(`/menu/create?id=${selectedMenuId}`)}
              ...
              <button
                ...
                className="rounded-lg border border-green-500 bg-green-50 px-3 py-1.5 text-xs font-medium text-green-700 transition hover:bg-green-100 dark:border-green-800 dark:bg-green-900/20 dark:text-green-400 disabled:opacity-50"
              >
          ...
          {/* Selectbox para escolher o cardápio */}
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-800 outline-none ring-0 ring-zinc-900/5 transition focus:border-zinc-400 focus:ring-2 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:focus:border-zinc-500 dark:focus:ring-zinc-600/40"
            >
              {savedMenus.map((menu) => (
                <option key={menu.id} value={menu.id}>
                  {menu.name} ({new Date(menu.createdAt).toLocaleDateString()})
                </option>
              ))}
            </select>
          </div>
