...
export default function MenuViewClient() {
  ...
  // Handle dynamic route: params.id is a string
  ...
  useEffect(() => {
    if (!menuId) {
      setError("ID do cardápio não fornecido");
      return;
    }
  ...
  const handleFileImport = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (menuId) {
        ...
        updateMenuVersion(menuId, menu.name, menu.items, menu.pricingRules);
        setMenuItems(menu.items);
        setPricingRules(menu.pricingRules);
        setMenuName(menu.name);
        setError(null);
        ...
        setError("ID do cardápio não encontrado.");
      }
    } catch (e) {
      const message =
